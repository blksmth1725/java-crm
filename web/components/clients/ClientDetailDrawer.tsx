...
import { useCreateInteraction, useInteractionsByClient } from "@/hooks/useInteractions"
import { useCreateTask, useTasksByClient } from "@/hooks/useTasks"
...
export function ClientDetailDrawer({
  ...
  const { data: interactions, isLoading: ixLoading } = useInteractionsByClient(clientId)
    ...
    createIx.mutate(
      {
        ...
        occurredAt: new Date(ixAt).toISOString(),
      },
      {
        onSuccess: () => {
        ...
        <div className="min-h-0 flex-1 space-y-4 py-4">
          ...
          {client && tab === "overview" && (
            <div className="space-y-3 text-sm">
              ...
              <p>
                <span className="text-muted-foreground">Region:</span> {client.region ?? "—"}
              </p>
              <p>
